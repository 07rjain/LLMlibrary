...
- JSON POST routes accept `application/json` and `application/*+json`, enforce a
  configurable 1 MiB default `maxBodyBytes`, and reject invalid bodies before
  store/client mutation.
- Session IDs are generated by the server. Trusted compatibility callers can
  set `allowClientSessionIds: true`; unknown message sessions return `404`
  unless `allowImplicitSessionCreate: true`.
...
The response contains the generated ID in `session.id`. Opted-in sequential
duplicate IDs return `409 session_already_exists` without replacement.
...
Metadata is validated and cloned before dispatch; cycles, accessors, custom
prototypes, undefined, BigInt, symbols, functions, and non-finite numbers are
rejected.
...
- `limit`
  Range `1..100`
- `direction`
  `forward` (default) or `backward`. Backward cursors are exclusive upper
  bounds and returned items remain chronological. Responses can include
...
- Missing trusted tenant context returns `403 tenant_context_required` before
  store access.
- Set `tenantResolution: 'single-tenant'` explicitly for global deployments.
