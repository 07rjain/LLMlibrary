...
import { parseGeminiRetryDelayMs, parseRetryAfterMs, withRetry, } from '../src/utils/retry.js';
describe('parseRetryAfterMs', () => {
    ...
    it('parses HTTP dates', () => {
        expect(parseRetryAfterMs('Tue, 15 Apr 2026 10:00:30 GMT', Date.parse('Tue, 15 Apr 2026 10:00:00 GMT'))).toBe(30000);
    });
...
describe('parseGeminiRetryDelayMs', () => {
    ...
    it('parses retryDelay objects', () => {
        expect(parseGeminiRetryDelayMs([{ retryDelay: { nanos: 500000000, seconds: 2 } }])).toBe(2500);
    });
...
describe('withRetry', () => {
    it('retries 429 responses using Retry-After', async () => {
        ...
        const sleep = vi.fn(async (_ms) => undefined);
        const response = await withRetry(async () => responses.shift(), {
            random: () => 0,
            sleep,
        });
