...
import { translateGeminiCacheCreateRequest, translateGeminiRequest, } from '../../src/providers/gemini.js';
...
liveDescribe('OpenAI – prompt caching (live)', () => {
    it('warms a cache key and observes cached tokens on the second request', async () => {
        ...
        expect(attempts.some((a, idx) => idx > 0 && a.cachedTokens > 0)).toBe(true);
    }, 60_000);
});
// ─── Anthropic ────────────────────────────────────────────────────────────────
...
liveDescribe('Anthropic – prompt caching (live)', () => {
    it('writes a cache entry on first request and reads it on the second', async () => {
        const client = LLMClient.fromEnv();
        const prefix = buildLargePrefix(`anthropic-droid-${Date.now()}`);
        const makeRequest = () => client.complete({
            maxTokens: 32,
            messages: [
                {
                    content: [
...
liveDescribe('Gemini – prompt caching (live)', () => {
    ...
    it('creates a cache, reads it back, updates TTL, lists it, and uses it in a completion', async () => {
...
liveDescribe('Cross-provider – all three caches in one run (live)', () => {
    it('confirms each provider independently reports cached tokens', async () => {
        const client = LLMClient.fromEnv();
        ...
        if (process.env.OPENAI_API_KEY) {
            try {
                ...
                const second = summarize(await client.complete({
                    maxTokens: 16,
                    messages: [{ content: `${prefix}\nReply OK.`, role: 'user' }],
                    model: 'gpt-4o-mini',
                    providerOptions: { openai: { promptCaching: { key: cacheKey, retention: 'in_memory' } } },
