...
import { createDenseRetriever, createPostgresKnowledgeStore, } from '../../src/retrieval.js';
...
liveDescribe('live embeddings smoke', () => {
    ...
    liveIt(Boolean(process.env.GEMINI_API_KEY))('embeds a text query with Gemini', async () => {
    ...
    liveIt(Boolean(process.env.GEMINI_API_KEY) && Boolean(process.env.DATABASE_URL))('stores an embedding in Postgres and retrieves it back through the dense retriever', async () => {
        const client = LLMClient.fromEnv({
            defaultEmbeddingModel: 'gemini-embedding-2',
        });
    ...
    liveIt(Boolean(process.env.GEMINI_API_KEY) && pdfLiveEnabled)('embeds a tiny PDF document when the dedicated PDF gate is enabled', async () => {
        const client = LLMClient.fromEnv({
            defaultEmbeddingModel: 'gemini-embedding-2',
        });
        const response = await client.embed({
