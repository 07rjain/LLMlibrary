...
describe('OpenAI adapter', () => {
    ...
    it('performs a text-to-speech request and returns audio bytes with estimated usage', async () => {
        const fetchImplementation = vi.fn(async () => new Response(new Uint8Array([1, 2, 3, 4]), {
    ...
    it('derives speech audio duration for pcm and wav output formats', async () => {
        const fetchImplementation = vi
            .fn()
            .mockResolvedValueOnce(new Response(bytesToArrayBuffer(new Uint8Array(48_000)), {
            status: 200,
        }))
            .mockResolvedValueOnce(new Response(bytesToArrayBuffer(makeWavBytes({ seconds: 2 })), {
    ...
    it('uses fallback speech media types and maps speech errors', async () => {
        const fetchImplementation = vi
            .fn()
            .mockResolvedValueOnce(new Response(bytesToArrayBuffer(new Uint8Array([1])), {
            status: 200,
        }))
    ...
    it('normalizes transcription text, verbose metadata, and provider options', async () => {
        ...
        const fetchImplementation = vi
            .fn()
            .mockResolvedValueOnce(new Response('plain transcript', {
        ...
        const verboseForm = fetchImplementation.mock.calls[1][1].body;
    ...
    it('streams two concurrent tool calls tracked independently by call_id', async () => {
        ...
        const startA = chunks.find((c) => c.type === 'tool-call-start' && c.id === 'call_a');
        const startB = chunks.find((c) => c.type === 'tool-call-start' && c.id === 'call_b');
        const deltaA = chunks.find((c) => c.type === 'tool-call-delta' && c.id === 'call_a');
        const deltaB = chunks.find((c) => c.type === 'tool-call-delta' && c.id === 'call_b');
        const resultA = chunks.find((c) => c.type === 'tool-call-result' && c.id === 'call_a');
        const resultB = chunks.find((c) => c.type === 'tool-call-result' && c.id === 'call_b');
