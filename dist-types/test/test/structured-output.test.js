...
import { buildAnthropicOutputConfig, normalizeStructuredSchema, parseStructuredOutput, } from '../src/structured-output.js';
describe('structured output helpers', () => {
    it('normalizes OpenAI strict schemas before dispatch', () => {
        expect(normalizeStructuredSchema({
            properties: {
                answer: { type: 'string' },
                count: { type: 'integer' },
            },
            required: ['answer'],
            type: 'object',
        }, 'openai', { root: true, strict: true })).toEqual({
    ...
    it('rejects intentionally unsupported v1 schema keywords', () => {
        expect(() => normalizeStructuredSchema({
            ...
            type: 'object',
        }, 'openai', { root: true, strict: true })).toThrow(ProviderCapabilityError);
    });
    ...
    it('keeps Anthropic json_object unsupported without a schema', () => {
        expect(() => buildAnthropicOutputConfig({ type: 'json_object' })).toThrow(ProviderCapabilityError);
    });
});
