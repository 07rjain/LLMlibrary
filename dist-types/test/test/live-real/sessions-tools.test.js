...
import { assertCanonicalResponse, collectStream, liveClient, liveRealEnabled, providerModels, requireLiveEnv, runId, weatherTool, } from './helpers.js';
...
liveDescribe('live-real sessions, tools, and context', () => {
    it('auto-executes tools inside Conversation and preserves history', async () => {
        ...
        const conversation = await client.conversation({
            ...
            provider: 'openai',
            sessionId,
            system: 'When weather is requested, use the tool. After receiving a tool result, answer with WEATHER_DONE and the city.',
    ...
    it('streams inside Conversation and persists final usage', async () => {
        ...
        const result = await collectStream(conversation.sendStream('Reply with exactly: CONVERSATION_STREAM_OK'));
    ...
    it('resolves implicit conversation routes before the real provider call', async () => {
        ...
        const response = await conversation.send('Reply with exactly: IMPLICIT_CONTEXT_OK');
        assertCanonicalResponse(response, 'openai');
    ...
    it('validates strict and permissive tool argument modes', async () => {
        ...
        const strict = new Conversation(LLMClient.mock({
            responses: [
                badToolResponse,
        ...
        let executed = false;
        const permissive = new Conversation(LLMClient.mock({
            responses: [
                badToolResponse,
    ...
    it('runs Session API Request/Response flows with tenant isolation', async () => {
        ...
        const createResponse = await apiA.handle(jsonRequest('POST', '/sessions', {
        ...
        const blockedTenantOverride = await apiA.handle(jsonRequest('GET', `/sessions?tenantId=${encodeURIComponent(tenantB)}`));
        expect(blockedTenantOverride.status).toBe(400);
        const sendResponse = await apiA.handle(jsonRequest('POST', `/sessions/${sessionId}/message`, {
        ...
        expect(listABody.sessions.items.some((item) => item.sessionId === sessionId)).toBe(true);
        const getB = await apiB.handle(jsonRequest('GET', `/sessions/${sessionId}`));
        expect(getB.status).toBe(404);
        const messages = await apiA.handle(jsonRequest('GET', `/sessions/${sessionId}/messages`));
        expect(messages.status).toBe(200);
        expect((await messages.json()).messages.items.length).toBeGreaterThan(0);
        const compact = await apiA.handle(jsonRequest('POST', `/sessions/${sessionId}/compact`, { maxMessages: 2 }));
        expect(compact.status).toBe(200);
        const fork = await apiA.handle(jsonRequest('POST', `/sessions/${sessionId}/fork`, {
            fromMessageIndex: 0,
            newSessionId: runId('fork'),
        }));
        expect(fork.status).toBe(201);
        const invalidJson = await apiA.handle(new Request('https://live-real.test/sessions', {
            body: '{bad',
            method: 'POST',
        }));
        expect(invalidJson.status).toBe(400);
        const missing = await apiA.handle(jsonRequest('POST', `/sessions/${sessionId}/message`, {}));
        expect(missing.status).toBeGreaterThanOrEqual(400);
        const deleted = await apiA.handle(jsonRequest('DELETE', `/sessions/${sessionId}`));
    ...
    it('trims context with sliding window and summarizes with a real LLM', async () => {
        ...
        const summarising = new SummarisationStrategy({
            ...
            summarizer: async (dropped) => {
                const response = await client.complete({
                    ...
                    messages: [
                        {
                            content: `Summarize these dropped messages in fewer than 12 words: ${JSON.stringify(dropped)}`,
                            role: 'user',
                        },
                    ],
                    model: providerModels.openai,
        ...
        expect(summarized.some((message) => message.metadata?.summary === true)).toBe(true);
    ...
    it('loads older snapshots that are missing reasoning totals safely', () => {
        ...
        const restored = Conversation.restore(LLMClient.mock(), snapshot);
