...
liveDescribe('live-real provider adapters', () => {
    it('uses OpenAI for completion, streaming, tools, vision, and errors', async () => {
        ...
        const tool = await client.complete({
            ...
            messages: [
                {
                    content: 'Use the get_weather tool for Paris. Do not answer from memory.',
                    role: 'user',
                },
            ],
            model,
    ...
    it('uses Anthropic for completion, streaming, tools, and errors', async () => {
        ...
        const tool = await client.complete({
            ...
            messages: [
                {
                    content: 'Use the get_weather tool for Berlin. Do not answer from memory.',
                    role: 'user',
                },
            ],
            model,
    ...
    it('uses Gemini for completion, streaming, tools, vision, thinking, and errors', async () => {
        ...
        const thinking = await withGeminiGenerateQuotaRetry(() => client.complete({
            ...
            messages: [
                {
                    content: 'Think briefly, then answer only the number: what is 19 + 23?',
                    role: 'user',
                },
            ],
    ...
    it('does not expose API keys in normalized provider errors', async () => {
        expect(hasEnv('OPENAI_API_KEY') || hasEnv('ANTHROPIC_API_KEY') || hasEnv('GEMINI_API_KEY')).toBe(true);
        const client = liveClient();
        try {
