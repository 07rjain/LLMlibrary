...
import { assertCanonicalResponse, assertBillableUsage, collectStream, hasEnv, liveClient, providerModels, requireLiveEnv, weatherTool, } from './live-real/helpers.js';
const enabled = process.env.LIVE_CONFORMANCE === '1';
const conformanceDescribe = enabled ? describe : describe.skip;
const providers = [
    { env: 'OPENAI_API_KEY', model: providerModels.openai, name: 'openai' },
    {
        env: 'ANTHROPIC_API_KEY',
        model: providerModels.anthropic,
        name: 'anthropic',
    },
    { env: 'GEMINI_API_KEY', model: providerModels.gemini, name: 'google' },
];
conformanceDescribe('live provider conformance', () => {
    for (const provider of providers) {
        it(`${provider.name} completes, streams, and reports usage`, async () => {
            ...
            expect(stream.chunks.map((chunk) => chunk.sequence)).toEqual(stream.chunks.map((_, index) => index + 1));
