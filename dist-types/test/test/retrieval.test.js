...
describe('retrieval helpers', () => {
    it('creates a dense retriever around embed() and the knowledge store', async () => {
        ...
        const searchByEmbedding = vi.fn(async () => [
            {
                chunkId: 'chunk-1',
                score: 0.93,
                sourceId: 'source-a',
    ...
    it('applies dense rerank hooks before final limiting', async () => {
        const rerank = vi.fn(async (results) => [...results].reverse());
        const retriever = createDenseRetriever({
            defaultTopK: 2,
            embed: async () => ({
                embeddings: [{ index: 0, values: [0.12, 0.34, 0.56] }],
    ...
    it('applies hybrid rerank hooks after candidate fusion', async () => {
        const rerank = vi.fn(async (results) => results.slice(0, 1));
        const retriever = createHybridRetriever({
            embed: async () => ({
    ...
    it('throws when a rerank hook returns a non-array result', async () => {
        const retriever = createDenseRetriever({
            ...
            rerank: async () => 'invalid',
            store: {
                searchByEmbedding: async () => [],
            },
        });
        await expect(retriever.search({ query: 'refund policy' })).rejects.toBeInstanceOf(LLMError);
    });
    ...
    it('formats retrieved context with citations and token limits', () => {
        ...
        expect(formatted.text).toContain('Score (raw retrieval score; not a probability): 0.9000');
    ...
    it('formats relative score display with explicit explanatory labels', () => {
        ...
        expect(formatted.text).toContain('Score (relative to top result; display-only, not a probability): 1.0000');
        expect(formatted.text).toContain('Score (relative to top result; display-only, not a probability): 0.5000');
    });
    it('falls back to explicit raw labels when relative display would be misleading', () => {
        ...
        expect(formatted.text).toContain('Score (raw lexical relevance; not a probability): 0.0000');
    });
    ...
    it('supports in-memory scope and metadata-array filters and can clear stored data', async () => {
        ...
        await store.clear();
        await expect(store.listKnowledgeSources({
            botId: 'bot-1',
            knowledgeSpaceId: 'space-1',
            tenantId: 'tenant-1',
        })).resolves.toEqual([]);
...
describe('in-memory knowledge store tenant isolation', () => {
    it('rejects a knowledge space upsert that reassigns an existing id to another tenant', async () => {
        ...
        await expect(store.upsertKnowledgeSpace({
            botId: 'bot-evil',
            id: 'space-1',
            name: 'Hijacked KB',
            tenantId: 'tenant-2',
    ...
    it('rejects a source upsert that crosses tenant or bot ownership', async () => {
        ...
        await expect(store.upsertKnowledgeSource({ ...base, tenantId: 'tenant-2' })).rejects.toThrow(/different tenant/);
        await expect(store.upsertKnowledgeSource({ ...base, botId: 'bot-2' })).rejects.toThrow(/different bot/);
    });
    it('rejects a chunk upsert that crosses tenant ownership', async () => {
        ...
        await expect(store.upsertKnowledgeChunk({ ...base, tenantId: 'tenant-2' })).rejects.toThrow(LLMError);
    });
});
