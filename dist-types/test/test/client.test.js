...
describe('LLMClient', () => {
    ...
    it('routes OpenAI speech calls, logs speech usage, and preserves metadata', async () => {
        ...
        const fetchImplementation = vi
            ...
            .mockResolvedValueOnce(new Response(JSON.stringify({ text: 'speech transcript' }), {
            headers: { 'content-type': 'application/json' },
            status: 200,
        }));
        const client = new LLMClient({
        ...
        expect(usageLogger.logSpeech).toHaveBeenCalledWith(expect.objectContaining({
            botId: 'bot-1',
            kind: 'speech',
            model: 'gpt-4o-mini-tts',
            provider: 'openai',
    ...
    it('handles speech validation, unsupported providers, and budget preflight', async () => {
        ...
        await expect(client.speak({ input: '' })).rejects.toBeInstanceOf(ProviderCapabilityError);
        await expect(client.speak({
            input: 'Hello',
            model: 'gpt-4o-mini-tts',
            provider: 'google',
        })).rejects.toBeInstanceOf(ProviderCapabilityError);
        ...
        const warnFetch = vi.fn(async () => new Response(new Uint8Array([1, 2]), {
            status: 200,
        }));
        ...
        expect(onWarning).toHaveBeenCalledWith(expect.stringContaining('Estimated speech request cost'));
    ...
    it('OpenAI conversation loop: store:false, no previous_response_id, full history re-sent every turn', async () => {
        ...
        const replayedAssistantMessage = secondInput.find((item) => item.role === 'assistant');
