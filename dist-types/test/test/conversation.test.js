...
describe('Conversation', () => {
    ...
    it('rejects prototype-sensitive tool argument keys in strict mode', async () => {
        ...
        const complete = vi
            ...
            toolCalls: [
                {
                    args: JSON.parse('{"__proto__":{"polluted":true},"city":"Berlin"}'),
                    id: 'tool_proto',
                    name: 'lookup_weather',
                },
                {
    ...
    it('requires own tool argument properties in strict mode', async () => {
        const execute = vi.fn(async () => ({ ok: true }));
        const inheritedArgs = Object.create({ city: 'Berlin' });
        const complete = vi
            .fn()
            .mockResolvedValueOnce({
            content: [],
    ...
    it('rejects non-finite or excessive tool loop limits', () => {
        ...
        expect(() => new Conversation(client, { maxToolRounds: Number.POSITIVE_INFINITY })).toThrow('maxToolRounds must be an integer between 0 and 100.');
        expect(() => new Conversation(client, { maxToolRounds: 101 })).toThrow('maxToolRounds must be an integer between 0 and 100.');
        expect(() => new Conversation(client, { toolExecutionTimeoutMs: Number.POSITIVE_INFINITY })).toThrow('toolExecutionTimeoutMs must be a finite number between 1 and 300000.');
        expect(() => Conversation.restore(client, { ...snapshot, maxToolRounds: Number.POSITIVE_INFINITY })).toThrow('maxToolRounds must be an integer between 0 and 100.');
        expect(() => Conversation.restore(client, { ...snapshot, toolExecutionTimeoutMs: Number.POSITIVE_INFINITY })).toThrow('toolExecutionTimeoutMs must be a finite number between 1 and 300000.');
    });
