...
import { redactPII, redactPIIFromMessages, redactPIIInJson, } from '../src/pii.js';
describe('PII redaction', () => {
    it('redacts email, phone, and Luhn-valid card values', () => {
        const result = redactPII('Email jane@example.com, call +1 (415) 555-2671, card 4242 4242 4242 4242.');
        expect(result.text).toBe('Email [REDACTED_EMAIL], call [REDACTED_PHONE], card [REDACTED_CREDIT_CARD].');
    ...
    it('does not treat ISO dates as phone numbers', () => {
        expect(redactPII('Created on 2026-07-14.').text).toBe('Created on 2026-07-14.');
    });
    ...
    it('supports configurable phone digit limits', () => {
        expect(redactPII('Extension 555-2671', {
            kinds: ['phone'],
            phone: { minDigits: 10 },
        }).text).toBe('Extension 555-2671');
        expect(() => redactPII('Call 555-2671', {
            kinds: ['phone'],
