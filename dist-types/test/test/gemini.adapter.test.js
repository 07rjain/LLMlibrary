...
describe('Gemini adapter', () => {
    ...
    it('uses Gemini 3.5 responseFormat envelope with enum mime type', () => {
        ...
        }).generationConfig.maxOutputTokens).toBeUndefined();
    });
    ...
    it('rejects cache names that could alter the authenticated request path', async () => {
        ...
        for (const name of malicious) {
            ...
            await expect(adapter.updateCache(name, { expireTime: '2026-04-21T12:00:00Z' })).rejects.toThrow(/Invalid Gemini cache name/);
    ...
    it('accepts valid cache names with or without the resource prefix', async () => {
        const fetchImplementation = vi.fn().mockImplementation(async () => new Response(JSON.stringify({ model: 'models/gemini-2.5-flash', name: 'cachedContents/cache_1' }), { headers: { 'content-type': 'application/json' }, status: 200 }));
