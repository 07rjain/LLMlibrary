...
export function assertResponseFormatSupported(model, responseFormat, options = {}) {
    if (!responseFormat || responseFormat.type === 'text') {
        return;
    }
    if (responseFormat.type === 'json_object' && !model.supportsJsonObjectOutput) {
        throw new ProviderCapabilityError(`Model "${model.id}" does not support JSON object output.`, {
            model: model.id,
            provider: model.provider,
        });
    }
    if (responseFormat.type === 'json_schema' && !model.supportsJsonSchemaOutput) {
        throw new ProviderCapabilityError(`Model "${model.id}" does not support JSON schema output.`, {
            model: model.id,
            provider: model.provider,
        });
    }
    if (options.stream && !model.supportsStructuredOutputStreaming) {
        throw new ProviderCapabilityError(`Model "${model.id}" does not support structured-output streaming requests.`, {
            model: model.id,
            provider: model.provider,
        });
    }
}
export function buildOpenAITextFormat(responseFormat, options) {
    if (!responseFormat || responseFormat.type === 'text') {
        return undefined;
    }
...
export function buildAnthropicOutputConfig(responseFormat) {
    ...
    if (responseFormat.type === 'json_object') {
        throw new ProviderCapabilityError('Anthropic JSON object output is not supported without a schema. Use responseFormat: { type: "json_schema", ... }.', {
            provider: 'anthropic',
        });
    }
    return {
...
export function parseStructuredOutput(response, responseFormat) {
    if (!responseFormat || responseFormat.type === 'text') {
        return response;
    }
...
export function normalizeStructuredSchema(schema, provider, options = {}) {
...
function assertOpenAIJsonInstruction(options) {
    ...
    if (!text.includes('JSON')) {
        throw new ProviderCapabilityError('OpenAI JSON object mode requires an instruction that includes the string "JSON".', {
            provider: 'openai',
        });
    }
}
function messageTextSegments(message) {
...
function assertSchemaLimits(schema) {
    const serialized = JSON.stringify(schema);
    if (serialized.length > MAX_SCHEMA_BYTES) {
        throw new ProviderCapabilityError(`Structured output schema exceeds ${MAX_SCHEMA_BYTES} bytes.`);
    }
    walkSchema(schema, {
        nodes: 0,
    });
}
function walkSchema(schema, stats, depth = 0) {
    stats.nodes += 1;
    if (stats.nodes > MAX_SCHEMA_NODES) {
        throw new ProviderCapabilityError(`Structured output schema exceeds ${MAX_SCHEMA_NODES} nodes.`);
    }
    if (depth > MAX_SCHEMA_DEPTH) {
        throw new ProviderCapabilityError(`Structured output schema exceeds depth ${MAX_SCHEMA_DEPTH}.`);
    }
    if (schema.properties && Object.keys(schema.properties).length > MAX_SCHEMA_PROPERTIES) {
        throw new ProviderCapabilityError(`Structured output schema object exceeds ${MAX_SCHEMA_PROPERTIES} properties.`);
    }
...
function normalizeSchemaNode(schema, provider, context) {
    ...
    if (provider === 'openai' && context.root && type !== 'object') {
        throw new ProviderCapabilityError('OpenAI JSON schema output requires a root object schema.', {
            provider,
        });
    }
    const normalized = {};
    ...
    if (isObjectSchemaType(type, schema.properties)) {
        if ((provider === 'openai' || provider === 'anthropic') &&
            ...
            throw new ProviderCapabilityError(`Structured output schema at ${context.path} uses object additionalProperties, which is not supported in v1.`, {
                provider,
            });
        }
    }
    else if (schema.additionalProperties !== undefined) {
        throw new ProviderCapabilityError(`Structured output schema at ${context.path} uses additionalProperties on a non-object schema.`, {
            provider,
        });
    }
    return normalized;
}
function rejectUnsupportedSchemaKeys(schema, path) {
    for (const key of Object.keys(schema)) {
        if (!SUPPORTED_SCHEMA_KEYS.has(key)) {
            throw new ProviderCapabilityError(`Structured output schema keyword "${key}" at ${path} is not supported in v1.`);
        }
    }
}
function normalizeSchemaType(type, path) {
    if (Array.isArray(type)) {
        throw new ProviderCapabilityError(`Structured output schema union type at ${path} is not supported in v1.`);
    }
    if (typeof type !== 'string') {
        return undefined;
    }
    if (type === 'null') {
        throw new ProviderCapabilityError(`Structured output schema null type at ${path} is not supported in v1.`);
    }
    return type;
}
