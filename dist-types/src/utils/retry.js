...
export function parseRetryAfterMs(retryAfter, nowMs = Date.now()) {
    if (!retryAfter) {
        return null;
    }
...
export function parseGeminiRetryDelayMs(details) {
    if (!details) {
        return null;
    }
...
export async function withRetry(fn, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        ...
        if (delayMs === null && response.status === 429) {
            delayMs = parseGeminiRetryDelayMs(await safelyReadGeminiErrorDetails(response.clone()));
        }
        if (delayMs === null) {
...
async function safelyReadGeminiErrorDetails(response) {
    try {
