...
export function stripHtml(input: string): string {
  ...
  const withStructure = input
    ...
    .replace(/<li\b[^>]*>/gi, '\n- ')
    .replace(
      /<\/(p|div|section|article|header|footer|aside|nav|ul|ol|table|thead|tbody|tfoot|tr|td|th|blockquote|pre|h[1-6])>/gi,
      '\n',
    )
    .replace(
      /<(p|div|section|article|header|footer|aside|nav|ul|ol|table|thead|tbody|tfoot|tr|blockquote|pre|h[1-6])\b[^>]*>/gi,
      '\n',
    )
    .replace(/<[^>]+>/g, ' ');
...
function skipLeadingWhitespace(input: string, startOffset: number): number {
