...
pnpm run ci                    # typecheck + lint + test + build (full gate)
```
...
`ModelRegistry` maps model ID strings (e.g. `"Codex-sonnet-4-6"`) to `ModelInfo` (provider, pricing, context window, capability flags). Pricing lives in `src/models/prices.json`. `pnpm pricecheck` validates freshness. Consumers can call `client.models.register()` to add custom models or `client.updatePrices()` for runtime overrides.
...
- `LLMClient` with `Codex-sonnet-4-6` as default model
