...
export function requireLiveEnv(name: string): void {
  if (!hasEnv(name)) {
    throw new Error(`${name} is required for this live-real test.`);
  }
}

export function runId(prefix = 'live'): string {
  return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2)}`;
}

export function liveClient(): LLMClient {
...
export function assertCanonicalResponse(
  ...
  expectedProvider: CanonicalProvider,
): void {
  ...
  expect(['stop', 'length', 'tool_call', 'content_filter', 'error']).toContain(
    response.finishReason,
  );
  assertUsage(response.usage);
}

export function assertUsage(usage: UsageMetrics): void {
...
export function strictJsonObject(value: unknown): JsonObject {
