...
describe('Usage logging', () => {
  ...
  it('captures DATABASE_URL through fromEnv()', () => {
    ...
    expect(
      (logger as unknown as { connectionString?: string }).connectionString,
    ).toBe('postgresql://example.test/usage');
  });
  ...
  it('throws when DATABASE_URL is missing for PostgresUsageLogger', async () => {
    ...
    await expect(logger.ensureSchema()).rejects.toThrow(
      'DATABASE_URL is required for PostgresUsageLogger.',
    );
  });
