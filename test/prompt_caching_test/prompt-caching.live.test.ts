...
liveDescribe('prompt caching live', () => {
  ...
  liveIt(Boolean(process.env.OPENAI_API_KEY))(
    'warms and reuses OpenAI prompt caching',
    async () => {
      ...
      expect(attempts.some((attempt, index) => index > 0 && attempt.cachedTokens > 0)).toBe(
        true,
      );
    },
    45_000,
  ...
  liveIt(Boolean(process.env.ANTHROPIC_API_KEY))(
    'writes and reads Anthropic prompt cache entries',
    async () => {
      const client = LLMClient.fromEnv();
  ...
  liveIt(Boolean(process.env.GEMINI_API_KEY))(
    'creates, reuses, updates, lists, and deletes a Gemini cache',
    async () => {
      const client = LLMClient.fromEnv();
