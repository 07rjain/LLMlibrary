...
import {
  ...
  type MockHttpServer,
} from './helpers/mock-http-server.js';
...
function handleOpenAIRequest(request: MockHttpRequest) {
...
function handleAnthropicRequest(request: MockHttpRequest) {
  const body = asRecord(request.json);
...
function handleGeminiRequest(request: MockHttpRequest) {
  const body = asRecord(request.json);
...
async function startServerOrSkip(
  handler: (request: MockHttpRequest) => ReturnType<typeof jsonResponse>,
): Promise<MockHttpServer | null> {
  try {
...
function extractAnthropicUserText(body: Record<string, unknown>): string {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  return messages
    .map((message) => {
      const entry = asRecord(message);
...
function extractGeminiUserText(body: Record<string, unknown>): string {
