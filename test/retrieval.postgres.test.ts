...
class MockPgPool implements PostgresKnowledgeStorePool {
  ...
  async query<TRow = Record<string, unknown>>(
    ...
    return result as PostgresKnowledgeStoreQueryResult<TRow>;
  }
}
...
describe('PostgresKnowledgeStore', () => {
  ...
  it('bootstraps the pgvector schema and indexes', async () => {
    ...
    expect(pool.queries.some((query) => query.text.includes('"public"."knowledge_spaces"'))).toBe(
      true,
    );
    expect(
      pool.queries.some((query) => query.text.includes('"public"."knowledge_chunks"')),
    ).toBe(true);
    expect(
