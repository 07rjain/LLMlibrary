...
describe('token estimator', () => {
  ...
  it('calls the Anthropic token count endpoint', async () => {
    ...
    const anthropicCalls = fetchImplementation.mock.calls as unknown as Array<
  ...
  it('calls the Gemini token count endpoint', async () => {
    const signal = new AbortController().signal;
    const fetchImplementation = vi.fn(async () =>
      new Response(JSON.stringify({ totalTokens: 456 }), {
        headers: { 'content-type': 'application/json' },
        status: 200,
      }),
    );
    ...
    const geminiCalls = fetchImplementation.mock.calls as unknown as Array<
      [RequestInfo | URL, RequestInit | undefined]
    >;
